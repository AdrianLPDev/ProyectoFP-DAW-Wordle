...
async function comprobarEstadisticas(hoy){
    ...
    await axios.post(`${urlBase}estadisticas.php`, data).then(respuesta => {setResult(respuesta.data);})
}
haJugado();
//Función principal de carga
async function haJugado(){
    await comprobarEstadisticas(hoy);
    if(resultado == true){//Si tiene jugada hoy una partida, le reenviamos a las estadísticas para que no pueda jugar
        ...
        */
         ...
         * 
        */
        var juegoAcabado = null;

        function getJuegoAcabado(){
            return juegoAcabado;
        }

        function setJuegoAcabado(value){
            juegoAcabado = value;
        }
        ...
        // RecogerTecla(evento)
        async function RecogerTecla(evento) {
                ...
                if (letra == "ENTER"){
                    ...
                    var res = await comprobarPalabra();
                    if (juegoAcabado) { //Si ha ganado, enviamos las estadísticas con los datos de victoria
                        añadirEstadisticas(numIntento, columnaIntento, true)
                        return
                    }else if (juegoAcabado == false){ //Si ha perdido, enviamos las estadísticas con los datos de derrota
                        añadirEstadisticas(numIntento, columnaIntento, false);
                        return
                    }else{//El juego sigue
                        return
                    }
         ...
         * 
         */
        async function recogerClick(letra) {
            //if(!juegoAcabado){
                /**
        ...
        function añadirEstadisticas(filaIntentos, columnaIntentos, juego){
            let intentos = ((filaIntentos+1));
            let victoria = 0;
            if (juego){//Si el juego es victoria, es decir, es igual a 'true', le damos el valor de 1
                victoria = 1;
            }
            registrarEstadisticas(usuario, hoy, victoria, intentos);
        }

        //Función para efectuar el registro del usuario
        function registrarEstadisticas(usuario, fecha, victoria, intentos){
            var data = new FormData();
            data.append('usuario', usuario);
            data.append('fecha', fecha);
            data.append('victoria', victoria);
            data.append('intentos', intentos);
            axios.post(`${urlBase}estadisticas.php`, data)
            .then(respuesta => {
